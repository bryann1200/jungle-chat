...
export function NewChatModal({
  ...
  async function create() {
    ...
    try {
      ...
      if (!myId || myId !== me) throw new Error("Your session expired — please sign in again.");

      // Supplying the id lets us avoid `.select()` here. Some valid RLS setups
      // allow the creator to insert a chat but only allow participants to read
      // it, and the participant rows do not exist until the next request.
      ...
      if (cErr) throw new Error(`Could not create chat: ${cErr.message}`);
      const memberIds = Array.from(new Set([myId, ...selected.filter((id) => id !== myId)]));
      const rows = memberIds.map((user_id) => ({ chat_id: chatId, user_id }));
      const { error: pErr } = await supabase.from("chatapp_chat_participants").insert(rows);
      if (pErr) {
        await supabase.from("chatapp_chats").delete().eq("id", chatId).eq("created_by", myId);
        throw new Error(`Chat was created, but members could not be added: ${pErr.message}`);
      }
      onCreated(chatId);
    } catch (err) {
